...
- Long-running tasks that download, convert, or otherwise process a long list must display ongoing real-time progress, persist progress, and support resuming after interruption without repeating completed items. Progress output must refresh as work completes and include completed work plus either a total count or a clearly stated indeterminate total.
- Long-running task progress displays may use appropriate third-party libraries, such as `tqdm`. Add any new dependency to the requirements file used by the consuming code.
